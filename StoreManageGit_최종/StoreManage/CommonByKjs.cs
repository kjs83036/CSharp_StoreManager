...
namespace StoreManage
{
    ...
    public class Employee
    {
        ...
        public string st_emp_id { set; get; }
        public string st_emp_password { set; get; }
        //매장코드, 소속매장
        public string st_code { set; get; }
        //직원 레벨
        ...
        public string st_emp_picture_path { set; get; }
    ...
    public class User_t
    {
        ...
        //요금제
        public string user_price_plan { set; get; }
        //요금제변경일
    ...
    public class St_emp_result
    {
        ...
        public string cash_sale_price { set; get; }
        //기타차감
        public string etc_offset { set; get; }
        //세후금액
        public string after_tax_price { set; get; }
        //마진
    ...
    public class Crud
    {
        ...
        public Crud(string connectionString)
        {
            ...
            this.tempCrudFunc = new TempCrudFunc();
        }
        ...
        public int Create_MS<T>(string query, T t)
        {
            ...
            using (SqlCommand sqlCmd = new SqlCommand(query, sqlConn))
            {
                ...
                PropertyInfo[] propertyInfos = tempCrudFunc.GetPropertyInfos(t);

                //get value 시 정보를 가져올 obj와 인덱싱번호 (없으면null)을 파라미터로 지정
                object[] values = tempCrudFunc.setValues(propertyInfos, t);
                tempCrudFunc.setSqlCommad(sqlCmd, columns, values);
        ...
        public int Create_MySql<T>(string query, T t)
        {
            ...
            using (MySqlCommand mySqlCmd = new MySqlCommand(query, mySqlConn))
            {
                ...
                //get value 시 정보를 가져올 obj와 인덱싱번호 (없으면null)을 파라미터로 지정
                object[] values = tempCrudFunc.setValues(propertyInfos, t);
                tempCrudFunc.setSqlCommad(mySqlCmd, columns, values);
                //sqltransition관련 함수, 오류시 transition rollback
                ...
                using (MySqlCommand mySqlCmd = new MySqlCommand(query, mySqlConn))
                {
                    ...
                    while (reader.Read())
                        ...
                        for (int i = 0; i < propertyInfo.Count(); i++)
                        {
                            ...
                            //프로퍼티를 활용하여 T객체에 필드 값 할당
                            if (readerValue.GetType() == typeof(DBNull))
                            {
                                propertyInfo[i].SetValue(tFromDB, null, null);
                                continue;
                            }
                            if (propertyInfo[i].PropertyType.Equals(typeof(int)))
                            {
                                propertyInfo[i].SetValue(tFromDB, Convert.ToInt32(readerValue, null));
                                continue;
                            }
                            
                            propertyInfo[i].SetValue(tFromDB, readerValue, null);
                        }
                    reader.Close();
                    mySqlConn.Close();
        ...
        public int Update_MS<T>(string query, T t)
        {
            ...
            using (SqlCommand sqlCmd = new SqlCommand(query, sqlConn))
            {
                ...
                PropertyInfo[] propertyInfos = tempCrudFunc.GetPropertyInfos(t);
                string[] columns = tempCrudFunc.setColumns(propertyInfos);
                object[] values = tempCrudFunc.setValues(propertyInfos, t);
                tempCrudFunc.setSqlCommad(sqlCmd, columns, values);
        ...
        //삭제 일반적으로 0 번째 값을 기준으로 지움, 특정 필드를 기준으로 지우고 싶으면 다른 필드의 값을 다 null로 바꾸면 됨.
        public int Delete_MySql<T>(string query, T t)
        {
            ...
            using (MySqlCommand mySqlCmd = new MySqlCommand(query, mySqlConn))
            {
                ...
                {
                    if (values[i] != null && typeof(DateTime) != values[i].GetType())
                    {
                        //@컬럼에 변수 할당, delete는 한개의 변수만 할당하므로 setSqlCommand를 쓰지않음
                        mySqlCmd.Parameters.AddWithValue(columns[i], values[i]);
                        result = sqlTran(mySqlConn, mySqlCmd);
                        return result;
                    }
                }
    ...
    public class DbFactory
    {
        ...
        public void addMysql(string connectionString)
        {
        ...
        public void addMssql(string connectionString)
        {
        ...
        //public void addOracle(string connectionString)
        //{
    ...
    public class TempFuncForWinform
    {
        ...
        public void clearTextBox(TextBox[] ArrayTextBoxs)
        {
            foreach ( var v in ArrayTextBoxs)
            {
                v.Text = "";
            }
        }
    ...
    public class Temp
    {
        public void clearTextFromArray(TextBox[] textBox)
        {
